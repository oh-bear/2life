...
import { SCENE_LOGIN_AREA } from '../../constants/scene'

import { USERS } from '../../network/Urls'
import HttpUtils from '../../network/HttpUtils'

const URL_code = USERS.code
const URL_reset_password = USERS.reset_password
...
export default class ResetPsw extends Component {
  ...
  async reset() {
    ...
    const res = await HttpUtils.post(URL_reset_password, data)
    if (res.code === 404) return this.setState({ showAccountTip: true, accountTip: '该号码未注册' })
    if (res.code === 405) return this.setState({ showCodeTip: true })
    if (res.code === 0) {
        Toast.success('重置成功', 1.5)
        Actions.pop()
    }
  }
    ...
    return (
      <TouchableWithoutFeedback onPress={dismissKeyboard}>
        <View style={styles.container} animation='fadeIn'>
          ...
          >
            <TextPingFang style={styles.text_btn}>重置密码</TextPingFang>
          </TouchableOpacity>
